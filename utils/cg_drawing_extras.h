...
inline void sphere_unit(GLfloat radius, GLint slices, GLint stacks) {
    for (int i = 0; i < stacks; ++i) {
        ...
        for (int j = 0; j < slices; ++j) {
            float phi1 = j * 2.0f * M_PI / slices;           
            float phi2 = (j + 1) * 2.0f * M_PI / slices;     
            ...
            GLfloat t1[2] = {j / (float)slices, i / (float)stacks};           
            GLfloat t2[2] = {j / (float)slices, (i + 1) / (float)stacks};     
            GLfloat t3[2] = {(j + 1) / (float)slices, (i + 1) / (float)stacks}; 
            GLfloat t4[2] = {(j + 1) / (float)slices, i / (float)stacks};     
...
inline void rect_texture_unit_aux_plat(float N, float M) {
    glTexCoord2f(0, M);
    glVertex3f(-0.5, -0.5, 0);
...
inline void cube_texture_unit_platform(float N, float M) {
    glEnable(GL_NORMALIZE);
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    ...
    glPushMatrix();
        ...
        glBegin(GL_QUADS);
            glNormal3f(0, 0, 1);
            rect_texture_unit_aux_plat(N, N);
        glEnd();
    glPopMatrix();
    ...
    glPushMatrix();
        ...
        glBegin(GL_QUADS);
            glNormal3f(0, 1, 0); 
            rect_texture_unit_aux_plat(M, M);
        glEnd();
    glPopMatrix();
